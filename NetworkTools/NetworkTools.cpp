...
namespace Plugin {
    ...
    SERVICE_REGISTRATION(NetworkTools, 1, 0);

    static Core::ProxyPoolType<Web::JSONBodyType< Core::JSON::ArrayType < NetworkTools::Data > > > jsonResponseFactory(1);

    /* virtual */ const string NetworkTools::Initialize(PluginHost::IShell* service)
    {
        string message;
    ...
    /* virtual */ void NetworkTools::Deinitialize(PluginHost::IShell* service)
    {
        _handler = nullptr;
    }

    /* virtual */ string NetworkTools::Information() const
    {
        // No additional info to report.
        return (EMPTY_STRING);
    }

    /* virtual */ void NetworkTools::Inbound(Web::Request& /* request */)
    {
    }

    /* virtual */ Core::ProxyType<Web::Response> NetworkTools::Process(const Web::Request& request)
    {
        ...
        if (request.Verb == Web::Request::HTTP_GET) {
            ...
            if (index.Next() == false) {
                ...
                if (index.Next() == false) {
                    ...
                    // Insert the requested channel with its status..
                    Data& element(response->Add());
                    Exchange::INetworkTools::mode mode = (index.Remainder() == _T("On")  ? 
                                                         Exchange::INetworkTools::ENABLED : 
                                                         (index.Remainder() == _T("Slave")   ? 
                                                          Exchange::INetworkTools::SLAVE      :
                                                          Exchange::INetworkTools::DISABLED) );
                    Consumer(name, mode);

                    element.Callsign = name;
                    element.Enabled  = (mode == Exchange::INetworkTools::ENABLED);
            ...
            result->ErrorCode = Web::STATUS_BAD_REQUEST;
            result->Message = _T("Unsupported request for the [NetworkTools] service.");
        }

        return result;
    }

    bool NetworkTools::ChannelExists(const string& name) const {
        PluginHost::VirtualInput::Iterator index (_handler->Consumers());
        while ( (index.Next() == true) && (index.Name() != name) ) /* INTENTIONALLY LEFT EMPTY */ ;
        return (index.IsValid());
    }

    RPC::IStringIterator* NetworkTools::Consumers() const /* override */ {
        return (Core::Service<RPC::StringIterator>::Create<RPC::IStringIterator>(_handler->Consumers().Container()));
    }

    bool NetworkTools::Consumer(const string& name) const /* override */ {
        return(_handler->Consumer(name));
    }

    uint32_t NetworkTools::Consumer(const string& name, const Exchange::INetworkTools::mode value) /* override */ {
        ImunityList::iterator index (std::find(_imunityList.begin(), _imunityList.end(), name));
        
        if (value == Exchange::INetworkTools::SLAVE) {
            ...
            _handler->Consumer(name, (value == Exchange::INetworkTools::ENABLED));
        }

        return (Core::ERROR_NONE);
    }

    uint32_t NetworkTools::Select(const string& name) /* override */ {
