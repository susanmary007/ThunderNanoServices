...
#include <AVS/SampleApp/SDKDiagnostics.h>
#endif
...
namespace Plugin {
    ...
    bool AVSDevice::Initialize(PluginHost::IShell* service, const string& configuration)
    {
        ...
        // TODO: cross check this is required or not
        ...
        const bool enableDiagnostic = config.EnableDiagnostic.Value();;
        if (enableDiagnostic == true) {
#if !defined(DIAGNOSTICS)
            TRACE(AVSClient, (_T("Requested DIAGNOSTICS, but it is not compiled in")));
            status = false;
#endif
        }

        if (status == true) {
            status = Init(audiosource, enableDiagnostic, enableKWD, pathToInputFolder, configJsonStreams);
        }

        return status;
    }

    bool AVSDevice::Init(const std::string& audiosource, const bool enableDiagnostic, const bool enableKWD, const std::string& pathToInputFolder, std::shared_ptr<std::vector<std::shared_ptr<std::istream>>> configJsonStreams)
    {
        ...
        if (!createMediaPlayersForAdapters(httpContentFetcherFactory, equalizerRuntimeSetup)) {
            TRACE(AVSClient,(_T("Could not create mediaPlayers for adapters")));
            return false;
        }
...
#if defined(DIAGNOSTICS)
        std::shared_ptr<SDKDiagnostics> diagnostics;
        if (enableDiagnostic == true) {
            diagnostics = SDKDiagnostics::create();
            if (!diagnostics) {
            ...
            if (diagnostics) {
                ...
                if (deviceProtocolTracer) {
                    ...
                    if (alexaClientSDK::avsCommon::utils::configuration::ConfigurationNode::getRoot()[DIAGNOSTICS_KEY].getInt(
                        ...
                        if (configMaxValue < 0) {
                            TRACE(AVSClient, (_T("ignoringMaxTracedMessages maxTracedMessages %d"), configMaxValue));
                        } else {
                    ...
                    if (alexaClientSDK::avsCommon::utils::configuration::ConfigurationNode::getRoot()[DIAGNOSTICS_KEY].getBool(
                        ...
                        if (configProtocolTraceEnabled) {
                            ...
                            TRACE(AVSClient, (_T("Protocol Trace has been enabled at startup")));
                        }
                    }
                }
            }
